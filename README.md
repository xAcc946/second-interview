...
| 项目 | 地址 |
|------|------|
| ① GitHub 仓库 | https://github.com/xAcc946/second-interview |
| ② GitHub Pages 在线作品 | https://xAcc946.github.io/second-interview/ |

> 在线作品在首次 Push 后由 GitHub Actions 自动构建部署，需数分钟，请稍后访问。
...
1. **登录** - 学号 + 密码登录页，未登录访问任意页面自动重定向到登录页（当前为演示阶段，任意非空学号/密码均可登入，登录态存 localStorage）
2. **26条信息聚合列表** - 卡片展示标题、来源（校级/院级/学生自发）、时间、地点、门槛、状态徽章
3. **状态 + 紧急度排序** - 自动计算报名中/即将截止/已截止可候补/已结束/长期开放，默认按「最该先动手」排序
4. **筛选 + 搜索** - 按类型、来源、「仅看可报名」「只看收藏」筛选，支持关键词搜索
5. **收藏 / 我要参加** - 收藏即时反馈；「我要参加」跳转报名表单（姓名/班别/学号，逐项校验），提交后展示报名成功页，可取消重报
6. **学生自主发布** - 表单校验（必填项 + 缺失信息提示），发布后立即进入同一列表、可被筛选到
7. **本地保存** - 登录态、收藏、报名信息、自主发布内容均存 localStorage，刷新/重开都保留
8. **详情展开** - 完整字段、缺失项标注「题目未提供」、关联信息提示
